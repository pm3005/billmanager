...
<div class="container-fluid">
	<form action="" id="update-status-form">
		...
		<div class="form-group">
			...
			<option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>Cargo Picked-Up</option>
			<option value="2" <?php echo isset($status) && $status == 2 ? 'selected' : '' ?>>Arrive at Station</option>
			<option value="3" <?php echo isset($status) && $status == 3 ? 'selected' : '' ?>>Stored at Warehouse</option>
			<option value="4" <?php echo isset($status) && $status == 4 ? 'selected' : '' ?>>Ready for Delivery</option>
...
<script>
	$(document).ready(function(){
		$('#update-status-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=update_shipment_status",
